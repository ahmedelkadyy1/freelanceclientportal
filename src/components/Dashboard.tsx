...
export default function Dashboard({ currentUser, token }: DashboardProps) {
  ...
  // Convert files cleanly to base64 for mockup in-memory saving
    ...
    reader.onloadend = async () => {
      try {
        ...
        if (res.ok) {
          ...
          showToast('File upload refused.', 'error');
        }
      } catch {
        showToast('File upload link failed.', 'error');
      }
    };
  ...
  return (
    <div id="dashboard-platform-wrapper" className="min-h-[calc(100vh-4rem)] flex flex-col lg:flex-row relative">
      ...
      <aside className="w-full lg:w-72 bg-white border-b lg:border-r border-slate-100 flex flex-col justify-between shrink-0 p-6">
        <div>
          {/* Section Heading info */}
          <div className="mb-6">
            <span className="text-[10px] font-mono tracking-widest text-slate-400 uppercase">Interactive Workspace</span>
            <h2 className="text-xl font-display font-medium text-slate-800 tracking-tight mt-1">
                      ...
                      <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-100">
                        ...
                        <div>
                          ...
                          <button
                            ...
                            className="text-xs font-mono bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-705 px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer"
                          >
